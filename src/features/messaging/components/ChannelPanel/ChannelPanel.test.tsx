...
describe("ChannelPanel states", () => {
  ...
  it("distinguishes loading failure from an empty channel list and retries", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "No channels." }),
    ...
    resolveSecond([]);
    await waitFor(() =>
      expect(state.setServerChannels).toHaveBeenCalledWith(1, []),
    );
  });
  ...
  it("keeps inline channel creation compact and keyboard-cancellable", () => {
    ...
    expect(
      screen.queryByRole("textbox", { name: "Channel name" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("makes owner deletion keyboard-discoverable without selecting the channel", async () => {
    ...
    const deleteButton = screen.getByRole("button", {
      name: "Delete channel general",
    });
    fireEvent.click(deleteButton);

    expect(state.setActiveChat).not.toHaveBeenCalled();
    expect(
      screen.getByRole("dialog", { name: "Delete Channel" }),
    ).toHaveTextContent('Delete "#general"?');
    fireEvent.click(screen.getByRole("button", { name: "Confirm" }));
    await waitFor(() =>
      expect(deleteRoomMock).toHaveBeenCalledWith("channel-8"),
    );
  });
  ...
  it("does not expose channel deletion to non-owners", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Delete channel general" }),
    ).not.toBeInTheDocument();
  });
});
