      ...
      <div className="w-[360px] min-w-[360px] flex-shrink-0 flex flex-col overflow-hidden h-full">
        ...
        <div className="p-3 border-b border-border bg-card flex-shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-primary to-primary/60 text-primary-foreground text-xs font-bold inline-flex items-center justify-center flex-shrink-0">{server?.name?.[0]?.toUpperCase() ?? "?"}</span>
            <span className="text-sm font-semibold whitespace-nowrap overflow-hidden text-ellipsis text-foreground">{server?.name ?? "Server"}</span>
          </div>
          {server && (
            <button
        ...
        <div className="flex items-center justify-between p-2.5 px-3.5 pb-1">
          <span className="text-2xs font-semibold uppercase tracking-wider text-muted-foreground/70">Channels</span>
          <button
            className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded hover:bg-accent"
        ...
        {/* Inline create form */}
        {showCreate && (
          <div className="p-2 border-b border-border flex-shrink-0 px-3">
            {createError && (
              <div className="bg-destructive/10 border border-destructive rounded-lg p-2 px-3 text-destructive text-xs mb-1.5">
                {createError}
              </div>
            )}
            <input
              className="w-full px-2.5 py-1.5 bg-background border border-border rounded-lg text-foreground text-sm font-inherit outline-none focus:border-primary focus-visible:ring-1 focus-visible:ring-ring"
              type="text"
            ...
            <div className="flex gap-1.5 mt-1.5">
              <button
                className="flex-1 px-4 py-1.5 bg-primary text-primary-foreground border-none rounded-lg text-sm font-semibold font-inherit cursor-pointer transition-colors duration-150 hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <button
                className="flex-1 px-4 py-1.5 bg-background border border-border rounded-lg text-muted-foreground text-sm font-inherit cursor-pointer transition-colors duration-150 hover:bg-accent"
        ...
        {/* Channel list */}
        <div className="flex-1 overflow-y-auto p-1.5 px-2 [scrollbar-gutter:stable]">
          {isLoading ? (
            <p className="p-3 text-muted-foreground/70 text-sm text-center leading-normal">Loading channels…</p>
          ) : !channels || channels.length === 0 ? (
            <p className="p-3 text-muted-foreground/70 text-sm text-center leading-normal">
              No channels yet.{" "}
              <button className="bg-none border-none text-primary cursor-pointer text-sm p-0 hover:underline" onClick={() => setShowCreate(true)}>
              ...
              {channels.map((ch) => {
                ...
                return (
                  <li key={ch.id} className="relative group">
                    <button
                      className={cn(
                        "flex items-center gap-1.5 w-full p-2 mx-0.5 border-none rounded-md bg-none text-muted-foreground cursor-pointer text-sm font-inherit text-left transition-colors duration-120 relative hover:bg-accent hover:text-foreground",
                    ...
                    >
                      ...
                      <span className="text-muted-foreground/70 text-base font-semibold flex-shrink-0">#</span>
                      <span className="whitespace-nowrap overflow-hidden text-ellipsis">{ch.name}</span>
                    </button>
                    {isOwner && (
                      <button
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-none border-none cursor-pointer text-muted-foreground/70 text-xs p-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100 hover:text-destructive"
                        onClick={(e) => {
                          e.stopPropagation();
