      ...
      <section aria-labelledby={`${titleId}-identity`} className="space-y-3 px-5 py-4">
        ...
        <div className="flex items-start gap-5">
          <div className="relative">
            <button type="button" aria-label="Change group photo" className="group relative h-[72px] w-[72px] shrink-0 overflow-hidden rounded-full border border-border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 [&>div]:h-full [&>div]:w-full" onClick={() => setAvatarMenuOpen((open) => !open)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setAvatarMenuOpen((open) => !open); } }}>
