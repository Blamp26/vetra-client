...
export function StickerPicker({ onSend, onClose }: { onSend: (id: string) => Promise<void>; onClose: () => void }) {
  ...
  const saveSession = useRef<{ key: string; packId?: string; mediaId?: string; stickerId?: string }>({ key: "" });
  ...
  const saveSticker = async (file: File, destination: StickerDestination, tags: string[]) => {
    const key = destination.kind === "existing" ? `existing:${destination.packId}` : `new:${destination.title}:${destination.visibility}`;
    if (saveSession.current.key !== key) saveSession.current = { key };
    ...
    if (!session.mediaId) {
      ...
      session.mediaId = (await postFormData<{ id: string }>("/media", form)).id;
    }
    if (!session.stickerId) {
      ...
      const sticker = await stickersApi.add(session.packId, { media_file_id: session.mediaId, width: image.naturalWidth, height: image.naturalHeight, format: file.type === "image/webp" ? "webp" : "png", emoji_tags: tags }) as unknown as StickerMessage;
