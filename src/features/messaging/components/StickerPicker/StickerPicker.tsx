...
import { StickerArtwork, stickerArtworkLabel } from "./StickerArtwork";
...
export function StickerPicker({ onSend, onInsertCustomEmoji, onSendGif, onClose, selectionRequest, onSelectionHandled }: { onSend: (id: string) => Promise<void>; onInsertCustomEmoji?: (emoji: StickerMessage) => void; onSendGif?: (gif: VetraGif) => Promise<void>; onClose: () => void; selectionRequest?: StickerPackSelectionRequest | null; onSelectionHandled?: (revision: number) => void }) {
  ...
  const [stickerRoot, setStickerRoot] = useState<HTMLDivElement | null>(null);
  ...
  const searchResults = useMemo(() => packs.flatMap((pack) => pack.stickers.map((sticker) => ({ ...sticker, pack }))).filter((sticker) => !normalizedQuery || `${sticker.pack.title} ${stickerArtworkLabel(sticker)}`.toLocaleLowerCase().includes(normalizedQuery)), [packs, normalizedQuery]);
  ...
  const saveSticker = async (prepared: PreparedSticker, destination: StickerDestination, tags: string[]) => {
    const destinationPack = packs.find((pack) => pack.id === destination.packId);
    const isCustomEmoji = destinationPack?.kind === "custom_emoji";
    ...
    if (!isCustomEmoji) await onSend(session.stickerId);
