...
export function LoginForm({ onSwitchToRegister }: Props) {
  ...
  return (
    <div className="bg-[#F8F8F8] border border-[#E1E1E1] rounded-[14px] p-8 w-full max-w-[400px]">
      <h2 className="text-[1.3rem] font-semibold mb-6">Welcome Back</h2>
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        {error && <div className="bg-[#E74C3C]/12 border border-[#E74C3C] rounded-lg p-2.5 px-3 text-[#E74C3C] text-[0.9rem]">{error.message}</div>}
        <label className="text-[0.78rem] font-semibold uppercase tracking-[0.06em] text-[#4A4A4A]" htmlFor="login-username">Username</label>
        <input
          ...
          required autoFocus
        />
        <label className="text-[0.78rem] font-semibold uppercase tracking-[0.06em] text-[#4A4A4A]" htmlFor="login-password">Password</label>
        <input
          className="w-full px-3 py-2.5 bg-white border border-transparent rounded-lg text-[#0A0A0A] text-[0.95rem] font-inherit outline-none transition-colors duration-150 focus:border-[#5865F2]"
      ...
      <p className="mt-4 text-center text-[0.9rem] text-[#4A4A4A]">
        ...
        <button className="bg-none border-none text-[#5865F2] cursor-pointer text-inherit p-0 hover:underline" onClick={onSwitchToRegister}>Register</button>
      </p>
    </div>
  );
}
