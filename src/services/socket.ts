...
export type ServerMemberRemovedHandler = (payload: {
  server_id: number;
  user_id: number;
}) => void;
...
export type RoomMemberRemovedHandler = (payload: {
  room_id: number;
  user_id: number;
}) => void;
export type RoomDeletedHandler = (payload: { room_id: number }) => void;
export type ChannelDeletedHandler = (payload: {
  server_id: number;
  channel_id: number;
}) => void;
export type RoomCreatedHandler = (room: any) => void;
export type GroupGovernanceChangedHandler = (payload: {
  room_id: number;
  user_id?: number | null;
...
export interface SocketManager {
  ...
  joinRoomChannel: (
    roomId: number,
    roomTopicRef?: ResourceRef,
  ) => Promise<void>;
  leaveRoomChannel: (roomId: number) => void;
  sendRoomMessageViaChannel: (
    roomId: number,
    payload: OutgoingMessagePayload,
  ) => Promise<Message>;
  ...
  onRoomMessageEdited: (
    roomId: number,
    handler: MessageEditedHandler,
  ) => () => void;
  onRoomMessageDeleted: (
    roomId: number,
    handler: MessageDeletedHandler,
  ) => () => void;
  onRoomReactionUpdated: (
    roomId: number,
  ...
  toggleReaction: (
    ...
    messageId: number,
    emoji: string,
  ) => Promise<ReactionUpdatedPayload>;
...
function hasSocketTicket(ticket: string | undefined): ticket is string {
  return typeof ticket === "string" && ticket.trim().length > 0;
}

async function resolveSocketAuthParams(
  token: string,
): Promise<SocketAuthParams> {
  try {
    const { socket_ticket } = await authApi.createSocketTicket();
...
export async function connectSocket(
  ...
  const refreshSocketAuthParams = () => {
    ...
    authRefreshInFlight = (async () => {
      authParams = await resolveSocketAuthParams(token);
    })().finally(() => {
  ...
  const serverMemberAddedBus = makeEventBus<{
    server_id: number;
    server_name: string;
    user_id: number;
    server: any;
  }>();
  const serverMemberRemovedBus = makeEventBus<{
    server_id: number;
    user_id: number;
  }>();
  ...
  const roomMemberRemovedBus = makeEventBus<{
    room_id: number;
    user_id: number;
  }>();
  const roomDeletedBus = makeEventBus<{ room_id: number }>();
  const groupGovernanceChangedBus = makeEventBus<{
    room_id: number;
    user_id?: number | null;
    actor_id?: number | null;
    event: string;
  ...
  userChannel.on("message_edited", (p: MessageEditedPayload) =>
    messageEditedBus.emit(p),
  );
  userChannel.on("message_deleted", (p: MessageDeletedPayload) =>
    messageDeletedBus.emit(p),
  );
  userChannel.on("reaction_updated", (p: ReactionUpdatedPayload) =>
    reactionBus.emit(p),
  );

  userChannel.on("presence_state", (p: PresenceState) => presStateBus.emit(p));

  userChannel.on("presence_diff", (p: PresenceDiff) => presDiffBus.emit(p));
  ...
  userChannel.on("server_member_removed", (p) =>
  ...
  return {
    ...
    onStatusUpdate: (h) =>
      statusBus.subscribe(({ ids, status }) => h(ids, status)),
    ...
    onLastSeen: (h) =>
      lastSeenBus.subscribe(({ userId, lastSeenAt }) => h(userId, lastSeenAt)),
    ...
    deleteMessage(recipientRef, messageId) {
      return new Promise((resolve, reject) => {
        userChannel
          ...
          .receive("error", (r) =>
            reject(new Error(r?.reason ?? "Delete failed")),
          )
    ...
    toggleDirectReaction(partnerRef, messageId, emoji) {
      return new Promise((resolve, reject) => {
        userChannel
          ...
          .receive("error", (r) =>
            reject(new Error(r?.reason ?? "Reaction failed")),
          )
    ...
    onRoomTypingStart: (roomId, h) =>
      ensureRoomBus(roomId).typingStart.subscribe(h),
    onRoomTypingStop: (roomId, h) =>
      ensureRoomBus(roomId).typingStop.subscribe(h),
    onRoomMessageEdited: (roomId, h) =>
      ensureRoomBus(roomId).messageEdited.subscribe(h),
    onRoomMessageDeleted: (roomId, h) =>
