 ...
 *
 * Принимает произвольную функцию загрузки страницы fetchPage(limit, beforeId?).
 * Для комнат limit передаётся, но может игнорироваться на стороне API —
 * важно только что PAGE_SIZE используется для определения hasMore.
 *
 * actions и fetchPage стабилизируются через useRef, чтобы не вызывать
 * лишние перезапуски эффектов при пересоздании объектов между рендерами.
 */
export function useMessagePagination(
  ...
  // Стабилизируем колбэки — Zustand-экшены уже стабильны,
  // но объект actions пересоздаётся при каждом рендере.
  ...
  // ── Начальная загрузка (выполняется один раз на каждый id) ────────────────
  useEffect(() => {
